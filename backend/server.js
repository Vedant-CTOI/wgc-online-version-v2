...
const auth = new GoogleAuth({
  ...
  // On hosting platforms (e.g. Render) a service-account key can be supplied
  // via the GOOGLE_CREDENTIALS env var as JSON; local dev keeps using ADC.
  ...(process?.env?.GOOGLE_CREDENTIALS ? { credentials: JSON.parse(process.env.GOOGLE_CREDENTIALS) } : {}),
});
...
// --- Static frontend serving (production hosting) ---
// Serves the built frontend (frontend/dist) from this same server so the app
// runs as a single web service on hosting platforms like Render. In local dev
// Vite serves the frontend instead and proxies /api-proxy + /ws-proxy here.
const FRONTEND_DIST = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', 'frontend', 'dist');
app.use(express.static(FRONTEND_DIST));
// SPA fallback — Express 5 removed string wildcards; use a RegExp with proxy
// paths excluded. Registered AFTER the API routes on purpose.
app.get(/^(?!\/api-proxy|\/ws-proxy).*/, (req, res) => {
